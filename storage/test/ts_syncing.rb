...
class TS_Syncing < Test::Unit::TestCase  
  ...
  def setup
    ...
    @policy_evens= ConflictPolicy.new(:type=> ConflictPolicyType::FUNC, 
    :func=>"Proc.new {|val1,val2| if val1.to_i%2==0 then return val1 else return val2 end}")
  end
  ...
  def test_sync_greater
    ...
    assert_equal(
      (1..4).map{|i| Record.new(:key => "0#{i}", :value => "val0#{i}")}.concat((5..9).map{|i| Record.new(:key => "0#{i}", :value => "val0#{i+1}")}), 
      record_list1)
  ...
  def test_sync_user_function
    ...
    (1..8).each do |i| # set some values for other server
    ...
    @server1.sync_set("syncfunc", to_sync,@server2.host, @policy_evens)
    ...
    record_list1 = @server1.get_set("syncfunc",desired) # vals 01,02,03,04,010,012,014,018
    assert_equal(
      (1..4).map{|i| Record.new(:key => "0#{i}", :value => "0#{i}")}.concat((5..8).map{|i| Record.new(:key => "0#{i}", :value => "0#{i*2}")}), 
      record_list1)
  
    record_list2 = @server2.get_set("syncfunc",desired) # vals 02,04,06,08,010,012,014,018
    assert_equal((1..8).map{|i| Record.new(:key => "0#{i}", :value => "0#{i*2}")},record_list2)
  end
