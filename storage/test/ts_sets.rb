...
class TS_Sets < Test::Unit::TestCase
  ...
  def test_range_limit_reverse
     ...
     record_list = @server.get_set("rangelimit",desired)
     assert_equal((6..8).map{|i| Record.new(:key => "0#{i}", :value => "val0#{i}")}, record_list)
    ...
    # should return first two values from values matching the range
     ...
     record_list = @server.get_set("rangelimit",desired)
     assert_equal((1..2).map{|i| Record.new(:key => "0#{i}", :value => "val0#{i}")}, record_list)
    ...
    record_list = @server.get_set("rangelimit", desired)
    assert_equal((8..9).map{|i| Record.new(:key=>"0#{i}", :value=>"val0#{i}")}, record_list);
  end
