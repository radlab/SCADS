...
case class MDCCFailureResult(var clientConfig: MDCCFailureWorkflowTask,
                             var loaderConfig: MDCCTpcwMicroLoaderTask,
                             var clusterAddress: String,
                             var clientId: Int) extends AvroPair {
  var startTime: String = _
  var times: Map[String, Seq[Int]] = null
}

case class MDCCFailureWorkflowTask(var numClients: Int,
  ...
  def run(): Unit = {
    ...
    val results = resultCluster.getNamespace[MDCCFailureResult]("FailureResults")
...
//      val itemIds = (1 to random.nextInt(3) + 1).map(_ => randomItem).toSet.toSeq
    ...
    val buckets = new ConcurrentHashMap[Int, ConcurrentLinkedQueue[Int]]()
    val bucketSize = 10
      ...
      while(endTime - iterationStartTime < (runTime + warmupTime)) {
        ...
        try {
          ...
          if (endTime - iterationStartTime > warmupTime) {
            ...
            if (actionCommit) {
              // Only record times for commits.
              val sinceStart = endTime - iterationStartTime - warmupTime
              val bucket = ((sinceStart / (bucketSize * 1000)) * bucketSize).toInt
              val bucketList = buckets.putIfAbsent(bucket, new ConcurrentLinkedQueue[Int]())

              bucketList.add(elapsedTime.toInt)
            }
          }
        } catch {
          case e => {
            logger.warning(e, "Execepting generating page: " + e)
          }
        }
      }
    })

    val res = MDCCFailureResult(this, loaderConfig, clusterRoot.canonicalAddress, clientId)
    res.startTime = startTime
    res.times = buckets.entrySet.toList.map(x => (x.getKey.toString, x.getValue.iterator.toList)).toMap

    logger.info("******** writing out aggregated results.")
    results ++= List(res)

    coordination.registerAndAwait("iteration1", numClients)
