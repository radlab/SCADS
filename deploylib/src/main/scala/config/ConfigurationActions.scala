...
trait ConfigurationActions {
  ...
  protected def createDirectory(target: RemoteMachine, directory: File): File = {
    target.executeCommand("mkdir -p " + directory)
    return directory
  }

  protected def createFile(target: RemoteMachine, filename: File, contents: String, mode: String): File = {
  ...
  protected def createRunitService(target: RunitManager, name: String, runCommand: String): RunitService = createRunitService(target, name, runCommand, "#!/bin/sh\nexec " + target.svlogdCmd + " -tt ./")

  protected def createRunitService(target: RunitManager, name: String, runCommand: String, logCommand: String): RunitService = {
  ...
  protected def createJavaService(target: RunitManager, localJar: File, className: String, maxHeapMb: Int, args: String): RunitService = {
