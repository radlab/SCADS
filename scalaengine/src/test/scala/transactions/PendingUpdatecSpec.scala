...
with BeforeAndAfterEach {
  ...
  private def singleLogicalUpdate(key: Int, delta: Int) = {
    val k = KeyRec(key, key.toString)
    val m = MDCCMetadata(0, List())
    val v = ValueRec("", delta)
    List(LogicalUpdate(keyBuilder.toBytes(k),
                       valueBuilder.toBytes(m, v)))
  }
  ...
  private def withVersionKeysDB(dbFactory: FactoryType, factory: FactoryType,
    ...
    p.accept(ScadsXid(1, 1), updates).nonEmpty should be (true)
  ...
  def pendingUpdates(dbFactory: FactoryType, factory: FactoryType,
    ...
    "read committed updates (VERSION)" in {
      ...
      p.accept(ScadsXid(2, 2), update).nonEmpty should be (true)
      ...
      p.accept(ScadsXid(3, 3), update2).nonEmpty should be (true)
    ...
    "accept new inserts for non-conflicting keys (VERSION)" in {
      ...
      p.accept(ScadsXid(4, 4), updatesMixed).nonEmpty should be (true)
    }
    ...
    "accept compatible logical updates" in {
      ...
      p.accept(ScadsXid(1, 1), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(2, 2), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(3, 3), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(4, 4), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(5, 5), singleLogicalUpdate(1, 1)).nonEmpty should be (true)

      p.commit(ScadsXid(5, 5), singleLogicalUpdate(1, 1)) should be (true)
      p.commit(ScadsXid(2, 2), singleLogicalUpdate(1, 1)) should be (true)
      p.commit(ScadsXid(3, 3), singleLogicalUpdate(1, 1)) should be (true)
      p.commit(ScadsXid(1, 1), singleLogicalUpdate(1, 1)) should be (true)
      p.commit(ScadsXid(4, 4), singleLogicalUpdate(1, 1)) should be (true)

      p.accept(ScadsXid(6, 6), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(7, 7), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(8, 8), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(9, 9), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      p.accept(ScadsXid(10, 10), singleLogicalUpdate(1, 1)).nonEmpty should be (true)
      ...
      b should not be (None)
      valueBuilder.fromBytes(b.get)._2 should be (Some(ValueRec("1", 8)))
    }
    ...
    "detect conflicts for pending updates (VERSION)" in {
      ...
      p.accept(ScadsXid(2, 2), updatesFirst).nonEmpty should be (false)

      // Conflicts with existing pending update, record numKeys - 1
      val updatesLast = singleVersionUpdate(numKeys - 1, numKeys - 1, 0)
      p.accept(ScadsXid(3, 3), updatesLast).nonEmpty should be (false)

      // Conflicts with existing pending update, record 0
      val updatesMixed = singleVersionUpdate(numKeys, numKeys, 0) :::
        singleVersionUpdate(0, 0, 0)
      p.accept(ScadsXid(4, 4), updatesMixed).nonEmpty should be (false)

      // Conflicts with existing pending update, all records
      p.accept(ScadsXid(5, 5), updates).nonEmpty should be (false)
    }
    ...
    "detect conflicts for committed updates (VERSION)" in {
      ...
      p.accept(ScadsXid(2, 2), update).nonEmpty should be (false)
    }
    ...
    "detect conflicts for mixed physical pending updates (VERSION + VALUE)" in {
      ...
      p.accept(ScadsXid(2, 2), update1).nonEmpty should be (true)

      // Conflict, because of the same key.
      val update2 = singleValueUpdate(0, 2, 0)
      p.accept(ScadsXid(3, 3), update2).nonEmpty should be (false)
      ...
      p.accept(ScadsXid(3, 3), update3).nonEmpty should be (true)

      // Conflict, because of the same key
      val update4 = singleVersionUpdate(0, 3, 2)
      p.accept(ScadsXid(4, 4), update4).nonEmpty should be (false)
    }
  ...
  "A PendingUpdateController" when {
    // These tests are slow, so just running one for now.
/*
