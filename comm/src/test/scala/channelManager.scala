...
object ChannelManagerSpec extends SpecificationWithJUnit("ChannelManager Specification") {
    class TestChannelManager extends NioAvroChannelManagerBase[TestMsg, TestMsg] {
        ...
        /* msg -> time received */
        private val received = new ConcurrentHashMap[Utf8,Long]
        ...
        def hasReceived(msgs: Utf8, time: Int):Boolean = hasReceived(List(msgs), time)

        def hasReceived(msgs: List[Utf8], time: Int): Boolean = {
    ...
    implicit def string2utf8(str: String):Utf8 = { new Utf8(str) }
