...
class ScadrLoader(val client: ScadrClient, 
   ...
   */
  private def randomInts(seed: Int, maxInt: Int, count: Int): Array[Int] = {
    require(maxInt >= 0)
    require(count >= 0)
   ...
   */
  private def randomStrings(seed: Int, count: Int): Array[String] = {
    require(count >= 0)
  ...
  private def stringSize(num: Int): Int = {
    var size = 256
    var length = 1
    while (size < num) {
      size = size * 256
