...
class ScadsMesosCluster(root: ZooKeeperProxy#ZooKeeperNode, scheduler: ServiceScheduler, initialSize: Int, cores: Int = 2, mem: Int = 5000) extends ScadsCluster(root) {
  val logger = Logger()
  ...
  val procDesc = JvmProcess(jarLocation.toString ,
    ...
    "--zooKeeper" :: root.proxy.address :: "--zooBase" :: root.name :: "--verbose" :: Nil,
