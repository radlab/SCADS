...
object AvroInFile {
  def apply[RecordType <: GenericContainer](file: File)(implicit schema: TypedSchema[RecordType]): DataFileReader[RecordType] with Iterator[RecordType] = {
    new DataFileReader[RecordType](file, new SpecificDatumReader[RecordType](schema)) with Iterator[RecordType]
  }
}

object AvroOutFile {
  def apply[RecordType <: GenericContainer](file: File, codec: Option[CodecFactory] = None)(implicit schema: TypedSchema[RecordType]): DataFileWriter[RecordType] = {
